...
(async()=>{
  ...
  await settled();
  const list=document.querySelector('#objects');list.value=editor.map.platforms[0].id;list.dispatchEvent(new Event('change'));
  const x=document.querySelector('[name=x]');const movedX=editor.map.platforms[0].x+16;x.value=String(movedX);x.dispatchEvent(new Event('change'));await settled();
  check(editor.map.platforms[0].x===movedX && runtime.game.scene.scenes[0].platforms[0].sprite.x===movedX,'Inspector and game collider disagree');
  check(JSON.parse(localStorage.getItem('chofis-map-editor-v1:fonda-plaza')).platforms[0].x===movedX,'Draft was not saved');
  ...
  localStorage.setItem('chofis-platformer-preview:fonda-plaza','["empanada"]');
  ...
  check(localStorage.getItem('chofis-platformer-preview:fonda-plaza')==='["empanada"]','Test changed real preview progress');
